...
public class Goomba : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.transform.CompareTag("Mushroom")) //if we touch Mushroom, change direction
        {
            Rigidbody2D colRB = collision.transform.GetComponent<Rigidbody2D>();
    ...
    {
        if ((collision.gameObject.CompareTag("Tiles") || collision.gameObject.CompareTag("Goomba")) && Mathf.Round(rig.velocity.x) == 0f) //if it's no longer going left/right - change direction
            isGoingRight = !isGoingRight;
    }
